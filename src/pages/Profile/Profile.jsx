      ...
      <div className="flex flex-col h-screen w-[80%] p-4 gap-4">
        <div className="flex flex-row items-center justify-between w-full py-4">
          ...
          <UserProfileContainer
            image={user?.avatar || "https://i.pinimg.com/1200x/f8/93/c7/f893c7e1abe5b076eaee7c58223329e5.jpg"}
            name={user ? `${user.name} ${user.surname}` : "Usuario"}
            label={user?.background?.profesion || "Profesión"}
          />
        </div>
        ...
        <div className="w-full h-[25%] rounded-2xl px-4 bg-(--color-alice-blue) shadow-sm">
          <AvatarCard
            image={user?.avatar || "https://i.pinimg.com/1200x/f8/93/c7/f893c7e1abe5b076eaee7c58223329e5.jpg"}
            name={user ? `${user.name} ${user.surname}` : "Usuario"}
            rol={user?.background?.profesion || "Profesión"}
            lugar={user ? `${user.city}, ${user.background?.pais || ""}` : "Ciudad, País"}
          />
        </div>
